...
const defaultImages = {
  ...
  giftVouchers: [
    {
      url: "https://images.unsplash.com/photo-1513475382585-d06e58bcb0e0?w=600&h=450&fit=crop&crop=center",
      filename: "gift-vouchers-main.jpg",
...
async function fetchProducts() {
  ...
  return new Promise((resolve, reject) => {
    ...
    https.get(url, (response) => {
      ...
      console.log('⚠️  Failed to fetch from Rezdy API, will use default images');
      resolve(null);
    });
  });
}
...
async function downloadHopOnHopOffImages(products) {
  ...
  if (products) {
    ...
    const productsWithImages = hopOnHopOffProducts.filter(product => 
    ...
    if (productsWithImages.length > 0) {
      ...
      for (const product of productsWithImages) {
        ...
        const imagesToDownload = product.images.slice(0, Math.min(2, 4 - imageCount));
        
        for (let i = 0; i < imagesToDownload.length; i++) {
          const image = imagesToDownload[i];
          const filename = generateFilename(product.name, imageCount + 1, image.largeSizeUrl, 'hop-on-hop-off');
          
          try {
            const localPath = await downloadImage(image.largeSizeUrl, filename, 'hop-on-hop-off');
  ...
  if (downloadedImages.length < 4) {
    console.log(`📥 Downloading ${4 - downloadedImages.length} default hop-on-hop-off images...`);
    
    const imagesToDownload = defaultImages.hopOnHopOff.slice(downloadedImages.length);
    
    for (const imageInfo of imagesToDownload) {
      try {
        const localPath = await downloadImage(imageInfo.url, imageInfo.filename, 'hop-on-hop-off');
        downloadedImages.push({
        ...
        console.error(`❌ Failed to download ${imageInfo.filename}:`, error.message);
      }
    }
  }
...
/**
 * Download Gift Voucher images
 */
async function downloadGiftVoucherImages(products) {
  console.log('\n🎁 Processing Gift Voucher images...');
  const downloadedImages = [];

  if (products) {
    // Filter gift voucher products
    const giftVoucherProducts = products.filter(product => 
    ...
    console.log(`🎯 Found ${giftVoucherProducts.length} gift voucher products`);

    // Find a product with images
    const productWithImage = giftVoucherProducts.find(product => 
      product.images && product.images.length > 0
    );

    if (productWithImage) {
      console.log(`📸 Found gift voucher product with images: ${productWithImage.name}`);
      
      const image = productWithImage.images[0];
      const filename = generateFilename(productWithImage.name, 1, image.largeSizeUrl, 'gift-voucher');
      
      try {
        const localPath = await downloadImage(image.largeSizeUrl, filename, 'gift-vouchers');
        downloadedImages.push({
          originalUrl: image.largeSizeUrl,
          localPath,
          filename,
          alt: image.caption || `${productWithImage.name} - Gift Voucher`
        });
      } catch (error) {
        console.error(`❌ Failed to download ${filename}:`, error.message);
      }
  ...
  // If we don't have an image from Rezdy, download default image
  if (downloadedImages.length === 0) {
    console.log('📥 Downloading default gift voucher image...');
    
    const imageInfo = defaultImages.giftVouchers[0];
    try {
      const localPath = await downloadImage(imageInfo.url, imageInfo.filename, 'gift-vouchers');
      downloadedImages.push({
        originalUrl: imageInfo.url,
        localPath,
        filename: imageInfo.filename,
...
async function downloadAllSectionImages() {
  try {
    ...
    // Download Gift Voucher images
    const giftVoucherImages = await downloadGiftVoucherImages(products);

    // Save metadata
    const metadata = {
      hopOnHopOff: hopOnHopOffImages,
      giftVouchers: giftVoucherImages,
      downloadedAt: new Date().toISOString()
    };

    // Save metadata to public directory
    const metadataPath = path.join(process.cwd(), 'public', 'section-images-metadata.json');
    fs.writeFileSync(metadataPath, JSON.stringify(metadata, null, 2));

    console.log('\n🎉 Download Summary:');
    console.log(`✅ Hop on Hop off images: ${hopOnHopOffImages.length}`);
    console.log(`✅ Gift voucher images: ${giftVoucherImages.length}`);
  ...
  downloadAllSectionImages()
    .then((metadata) => {
      if (metadata) {
        ...
        console.log('   • /public/gift-vouchers/ (1 image)');
